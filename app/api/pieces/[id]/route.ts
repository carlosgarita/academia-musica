...
export async function GET(
  ...
  try {
    ...
    console.error("Unexpected error in GET /api/pieces/[id]:", error);
    return NextResponse.json(
      {
...
export async function PATCH(
  ...
  try {
    ...
    // Verify piece exists and user has access
    const { data: existingPiece, error: fetchError } = await supabaseAdmin
      .from("songs")
      .select("academy_id")
      .eq("id", params.id)
      .is("deleted_at", null)
    ...
    if (
      profile.role !== "super_admin" &&
      existingPiece.academy_id !== profile.academy_id
    ) {
      return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    }
    ...
    console.error("Unexpected error in PATCH /api/pieces/[id]:", error);
    return NextResponse.json(
      {
        error: "An unexpected error occurred",
        details: error instanceof Error ? error.message : "Unknown error",
...
export async function DELETE(
  ...
  try {
    ...
    console.error("Unexpected error in DELETE /api/pieces/[id]:", error);
    return NextResponse.json(
      {
        error: "An unexpected error occurred",
        details: error instanceof Error ? error.message : "Unknown error",
