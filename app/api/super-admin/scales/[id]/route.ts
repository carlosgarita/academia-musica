...
export async function PATCH(
  ...
  if (numeric_value !== undefined) updates.numeric_value = Number(numeric_value);
  ...
  const { data, error } = await auth.supabaseAdmin
    ...
    .single();
  if (error) {
    console.error("PATCH /api/super-admin/scales/[id]:", error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
  if (!data) return NextResponse.json({ error: "Escala not found" }, { status: 404 });
...
export async function DELETE(
  ...
  const { error } = await auth.supabaseAdmin
    ...
    .eq("id", id);
  if (error) {
    console.error("DELETE /api/super-admin/scales/[id]:", error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
