...
export default function EditBadgePage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/super-admin/badges`)
      .then((r) => r.json())
      .then((d) => {
        const badge = (d.badges || []).find((b: { id: string }) => b.id === id);
        if (badge) {
          setName(badge.name);
          setDescription(badge.description || "");
          setImageUrl(badge.image_url || "");
          setVirtud(badge.virtud || "");
          setFrase(badge.frase || "");
          setDisplayOrder(badge.display_order ?? 0);
        } else setError("Badge no encontrado");
      })
  ...
  return (
    <div className="max-w-xl space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-gray-900">Editar Badge</h1>
      </div>
