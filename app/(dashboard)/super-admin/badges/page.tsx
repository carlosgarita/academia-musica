...
export default function SuperAdminBadgesPage() {
  ...
  async function handleDelete(id: string, name: string) {
    if (!confirm(`¿Eliminar el badge "${name}"?`)) return;
    try {
      const res = await fetch(`/api/super-admin/badges/${id}`, { method: "DELETE" });
      if (!res.ok) {
  ...
  if (loading) return <p className="text-gray-600">Cargando badges...</p>;
    ...
    <div className="space-y-6">
      ...
      {badges.length === 0 ? (
        <div className="rounded-lg border-2 border-dashed border-gray-300 p-12 text-center">
          ...
          <p className="mt-2 text-gray-600">No hay badges creados.</p>
          <Link
            href="/super-admin/badges/new"
            className="mt-4 inline-flex text-indigo-600 hover:text-indigo-500"
          >
            Crear el primer badge
          </Link>
        </div>
      ) : (
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Virtud</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {badges.map((b) => (
                <tr key={b.id}>
                  <td className="px-4 py-3 text-sm text-gray-500">{b.display_order}</td>
                  <td className="px-4 py-3 text-sm font-medium text-gray-900">{b.name}</td>
                  <td className="px-4 py-3 text-sm text-gray-500">{b.virtud || "—"}</td>
                  <td className="px-4 py-3">
                    {b.image_url ? (
                      <img src={b.image_url} alt="" className="h-8 w-8 rounded object-cover" />
                    ) : (
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link
                      href={`/super-admin/badges/${b.id}/edit`}
