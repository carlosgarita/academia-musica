...
type Professor = { id: string; first_name: string | null; last_name: string | null; email?: string };
...
export default function NewCoursePage() {
  ...
  useEffect(() => {
    if (subjectId) {
      ...
      fetch(`/api/professors?subject_id=${subjectId}`)
        .then((r) => r.json())
        .then((d) => { setProfessors(d.professors || []); setProfileId(""); })
  ...
  function addTurno() {
    ...
    const dup = turnos.some(
      (t) => t.day_of_week === newDay && t.start_time === newStart && t.end_time === newEnd
    );
    if (dup) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (sessionDates.length === 0) {
      setError("Genera y agrega al menos una fecha de sesión antes de crear el curso");
      setLoading(false);
      return;
    }
    if (turnos.length === 0) {
    ...
    try {
      ...
      if (!r.ok) {
        throw new Error(d.details ? `${d.error}: ${d.details}` : d.error || "Error al crear el curso");
      }

      router.push("/director/direccion/courses");
    } catch (e) {
      setError(e instanceof Error ? e.message : "Error inesperado al crear el curso");
      setLoading(false);
    }
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Crea un curso con su materia, profesor, periodo, sesiones (rango de fechas) y turnos (días y horarios)
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <select
              ...
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
            >
              <option value="">{loadingSubjects ? "Cargando..." : "Selecciona una materia"}</option>
              {subjects.map((s) => (
          ...
          <div>
            ...
            >
              <option value="">
                {!subjectId ? "Primero elige una materia" : loadingProfessors ? "Cargando..." : "Selecciona un profesor"}
              </option>
              {professors.map((p) => (
        ...
        <div className="border-t border-gray-200 pt-6">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
            ...
            <div className="flex items-end gap-2">
              {editingId ? (
                <>
                  <button type="button" onClick={addTurno} className="flex-1 rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">
                    Guardar
                  </button>
                  <button type="button" onClick={cancelEdit} className="rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50">
                    Cancelar
                  </button>
                </>
              ) : (
                <button type="button" onClick={addTurno} className="w-full rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">
                  Agregar turno
                </button>
              )}
            </div>
                ...
                .map((t) => (
                  <li key={t.id} className="flex items-center justify-between rounded-md border border-gray-200 bg-gray-50 px-4 py-2">
                    <span className="text-sm">
                      {DAY_NAMES[t.day_of_week - 1]} {t.start_time.slice(0, 5)} – {t.end_time.slice(0, 5)}
                    </span>
