...
export function EditGuardianForm({
  ...
  async function loadGuardianData() {
    try {
      ...
      setAdditionalInfo(guardian.additional_info || "");
      setStatus(guardian.status || "active");
      ...
      if (assignmentsResponse.ok) {
        ...
        const assignedIds = (assignmentsData.assignments || []).map(
      ...
      setError(
        err instanceof Error ? err.message : "Error al cargar el encargado"
      );
    } finally {
    ...
    try {
      ...
      const response = await fetch(`/api/guardians/${guardianId}`, {
        ...
        body: JSON.stringify({
          ...
          student_ids: selectedStudentIds, // Update student assignments
        }),
      });
  ...
  if (isLoadingData) {
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-gray-600">Cargando encargado...</div>
      </div>
    );
  }
