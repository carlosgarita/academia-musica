...
create index idx_course_registrations_academy_id on public.course_registrations(academy_id);
create index idx_course_registrations_student_id on public.course_registrations(student_id);
create index idx_course_registrations_period_id on public.course_registrations(period_id);
...
create table public.course_registration_songs (
  id uuid default gen_random_uuid() primary key,
...
create index idx_course_registration_songs_registration on public.course_registration_songs(course_registration_id);
create index idx_course_registration_songs_song on public.course_registration_songs(song_id);
...
create policy "Super admins can do everything with course_registrations"
  on public.course_registrations as permissive for all to authenticated
  using (exists (select 1 from public.profiles where profiles.id = auth.uid() and profiles.role = 'super_admin'));

create policy "Directors can manage course_registrations in their academy"
  on public.course_registrations as permissive for all to authenticated
  using (exists (select 1 from public.profiles where profiles.id = auth.uid() and profiles.role = 'director' and profiles.academy_id = course_registrations.academy_id))
  with check (exists (select 1 from public.profiles where profiles.id = auth.uid() and profiles.role = 'director' and profiles.academy_id = course_registrations.academy_id));

create policy "Professors can view course_registrations in their academy"
  on public.course_registrations as permissive for select to authenticated
  using (exists (select 1 from public.profiles where profiles.id = auth.uid() and profiles.role = 'professor' and profiles.academy_id = course_registrations.academy_id));

create policy "Students can view own course_registrations"
  on public.course_registrations as permissive for select to authenticated
  using (exists (select 1 from public.students where students.user_id = auth.uid() and students.id = course_registrations.student_id));

create policy "Guardians can view course_registrations for their children"
  on public.course_registrations as permissive for select to authenticated
  using (exists (select 1 from public.guardian_students where guardian_students.guardian_id = auth.uid() and guardian_students.student_id = course_registrations.student_id));

-- Course registration songs policies
create policy "Super admins can do everything with course_registration_songs"
  on public.course_registration_songs as permissive for all to authenticated
  using (exists (select 1 from public.profiles where profiles.id = auth.uid() and profiles.role = 'super_admin'));

create policy "Directors can manage course_registration_songs"
  on public.course_registration_songs as permissive for all to authenticated
  using (exists (select 1 from public.profiles inner join public.course_registrations on course_registrations.id = course_registration_songs.course_registration_id where profiles.id = auth.uid() and profiles.role = 'director' and profiles.academy_id = course_registrations.academy_id))
  with check (exists (select 1 from public.profiles inner join public.course_registrations on course_registrations.id = course_registration_songs.course_registration_id where profiles.id = auth.uid() and profiles.role = 'director' and profiles.academy_id = course_registrations.academy_id));

create policy "Professors can view course_registration_songs"
  on public.course_registration_songs as permissive for select to authenticated
  using (exists (select 1 from public.profiles inner join public.course_registrations on course_registrations.id = course_registration_songs.course_registration_id where profiles.id = auth.uid() and profiles.role = 'professor' and profiles.academy_id = course_registrations.academy_id));

create policy "Students can view own course_registration_songs"
  on public.course_registration_songs as permissive for select to authenticated
  using (exists (select 1 from public.students inner join public.course_registrations on course_registrations.id = course_registration_songs.course_registration_id where students.user_id = auth.uid() and students.id = course_registrations.student_id));

create policy "Guardians can view course_registration_songs for their children"
  on public.course_registration_songs as permissive for select to authenticated
  using (exists (select 1 from public.guardian_students inner join public.course_registrations on course_registrations.id = course_registration_songs.course_registration_id where guardian_students.guardian_id = auth.uid() and guardian_students.student_id = course_registrations.student_id));
