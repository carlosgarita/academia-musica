...
export function AulaBadgeAssignment({
  ...
  const handleRemove = async (badgeId: string) => {
    ...
    try {
      const r = await fetch(
        `/api/student-badges?course_registration_id=${encodeURIComponent(registrationId)}&badge_id=${encodeURIComponent(badgeId)}`,
        { method: "DELETE" }
      );
      if (!r.ok) {
        const d = await r.json();
  ...
  const notAssigned = availableBadges.filter((b) => !assignedBadges.some((ab) => ab.badge_id === b.id));
                ...
                {assignedBadges.map((b) => (
                  <div key={b.id} className="flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-2.5 py-2">
                    ...
                    <div>
                      ...
                      {b.virtud && <p className="text-xs text-indigo-600">{b.virtud}</p>}
                    </div>
                    <button
                      type="button"
                      onClick={() => handleRemove(b.badge_id)}
                      disabled={removingBadgeId !== null}
                      className="ml-1 text-red-500 hover:text-red-600 disabled:opacity-50 p-0.5"
                      title="Quitar badge"
                    >
                      <X className="h-4 w-4" />
                    </button>
