    ...
    <div className="mt-2 w-full basis-full shrink-0 rounded-lg border border-gray-200 bg-gray-50/50 p-4">
      ...
      {loading ? (
        ...
        <p className="text-sm text-gray-500">
          No hay rubros de evaluación configurados. Configura rubros en la
          materia.
        </p>
      ) : (
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            <thead>
              <tr>
                <th className="py-2 pr-4 text-left font-bold text-gray-700">
                  Canción
                </th>
                {rubrics.map((r) => (
                  <th
                    key={r.id}
                    className="py-2 px-2 text-left font-bold text-gray-700 whitespace-nowrap"
                  >
                    {r.name}
                  </th>
                ))}
                  ...
                  {rubrics.map((rubric) => (
                    <td key={rubric.id} className="py-2 px-2">
                      <select
                        ...
                        onChange={(e) =>
                          handleScaleChange(song.id, rubric.id, e.target.value)
                        }
                        disabled={saving !== null}
                        className="block w-full max-w-[180px] rounded border border-gray-300 bg-white text-sm focus:border-indigo-500 focus:ring-indigo-500"
                      >
                        <option value="">—</option>
                        {scales.map((s) => (
