...
export function AulaSessionStudents({
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (!regRes.ok) throw new Error(regData.error || regData.details || "Error al cargar estudiantes");
  ...
  const handleSave = async (regId: string) => {
    ...
    try {
      ...
      if (!attendanceStatus) {
        ...
        setAttendances((prev) => { const next = { ...prev }; delete next[regId]; return next; });
      } else {
        const attRes = await fetch("/api/session-attendances", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ course_registration_id: regId, period_date_id: sessionId, attendance_status: attendanceStatus }),
        });
        if (!attRes.ok) {
      ...
      if (!commentText) {
        ...
        setComments((prev) => { const next = { ...prev }; delete next[regId]; return next; });
        setSavedCommentIds((prev) => { const next = new Set(prev); next.delete(regId); return next; });
      } else {
        const comRes = await fetch("/api/session-comments", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ course_registration_id: regId, period_date_id: sessionId, comment: commentText }),
        });
        if (!comRes.ok) {
      ...
      if (!assignmentText) {
        ...
        setAssignments((prev) => { const next = { ...prev }; delete next[regId]; return next; });
        setSavedAssignmentIds((prev) => { const next = new Set(prev); next.delete(regId); return next; });
      } else {
        const assRes = await fetch("/api/session-assignments", {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ course_registration_id: regId, period_date_id: sessionId, assignment_text: assignmentText }),
        });
        if (!assRes.ok) {
  ...
  if (loading) return <p className="text-gray-500 text-sm">Cargando estudiantes...</p>;
  if (error) return <p className="text-amber-600 text-sm">{error}</p>;
  if (registrations.length === 0) {
    return (
      <p className="text-gray-500 text-sm">
        No hay estudiantes matriculados en este curso. Gestiona matrículas en Dirección → Matrículas.
      </p>
    );
  }
    ...
    <>
      <ul className="space-y-4">
        {registrations.map((reg) => (
          <li key={reg.id} className="py-5 px-3 -mx-3 rounded-lg border border-gray-200 bg-white hover:border-gray-300">
            <div className="flex items-center gap-4">
              ...
              <Link
                href={`${base}/estudiante/${reg.id}${sessionId ? `?sesion=${sessionId}` : ""}`}
            ...
            <div className="mt-4 pl-14">
              ...
              <select
                ...
                onChange={(e) => setAttendances((prev) => ({ ...prev, [reg.id]: e.target.value }))}
                ...
                {savedCommentIds.has(reg.id) && editingCommentFor !== reg.id ? (
                  <div className="rounded-md border border-gray-200 bg-gray-50 px-3 py-2">
                    ...
                    <div className="mt-2 flex gap-2">
                      <button type="button" onClick={() => setEditingCommentFor(reg.id)} className="inline-flex items-center gap-1 text-xs text-indigo-600 hover:text-indigo-700">
                        <Pencil className="h-3.5 w-3.5" /> Editar
                      </button>
                      <button type="button" onClick={() => handleDeleteComment(reg.id)} className="inline-flex items-center gap-1 text-xs text-red-600 hover:text-red-700">
              ...
              <div className="mt-0.5">
                {savedAssignmentIds.has(reg.id) && editingAssignmentFor !== reg.id ? (
                  <div className="rounded-md border border-gray-200 bg-gray-50 px-3 py-2">
                    ...
                    <div className="mt-2 flex gap-2">
                      <button type="button" onClick={() => setEditingAssignmentFor(reg.id)} className="inline-flex items-center gap-1 text-xs text-indigo-600 hover:text-indigo-700">
                        <Pencil className="h-3.5 w-3.5" /> Editar
                      </button>
                      <button type="button" onClick={() => handleDeleteAssignment(reg.id)} className="inline-flex items-center gap-1 text-xs text-red-600 hover:text-red-700">
                        <Trash2 className="h-3.5 w-3.5" /> Eliminar
                      </button>
                    </div>
                  </div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => setAssignments((prev) => ({ ...prev, [reg.id]: e.target.value }))}
                    disabled={savingId === reg.id}
                    rows={2}
                    maxLength={1500}
                    className="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm"
            ...
            <div className="mt-4 pl-14">
              ...
              >
                {savingId === reg.id ? "Guardando..." : (<><Save className="h-4 w-4" /> Guardar</>)}
              </button>
            </div>
      ...
      {snackbar.show && (
        <div role="status" aria-live="polite" className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 rounded-lg bg-gray-900 text-white px-4 py-2.5 text-sm shadow-lg">
          {snackbar.message}
        </div>
      )}
    </>
